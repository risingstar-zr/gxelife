...
//            wiz_prepareAllFrames(win);
    ...
    function launchClientClipperArticle(info) {
        var params;
        if (!info.isNative) {
            params = wiz_getSelectedServer(window, info);
        } else {
            params = wiz_collectAllFrames(window, info) + wiz_getSelected(window);
        }
        info.params = params;
    ...
    function launchClientClipperFullPage(info) {
        var body, params;
        body = getFullpageHTML();
        if (!info.isNative) {
            params = wiz_collectAllFrames(window, info);
        } else {
            params = wiz_collectAllFrames(window, info) + formatParams(info.url, body);
        }
        info.params = params;
    ...
    function launchClientClipperSelection(info) {
        ...
        if (info.isNative) {
            params = wiz_collectAllFrames(window, info) + formatParams(info.url, bodyHTML);
        } else {
    ...
    function getFullpageHTML() {
        ...
        var pattern = /^<head[^>]*></i;
        var index = pattern.exec(page_content);
        var docType = document.doctype;
        if (!!docType && !docType.systemId && !docType.publicId) {
            docType = '<!DOCTYPE HTML>';
        } else if (!!docType) {
        ...
        var fullpage = [];
        //如果head头有id或者其他内容“page_content.substring(0, index + 6)”错误
        fullpage.push(docType + '<html>', page_content.substring(0, index), base, page_content.substring(index), '</html>');
...
//            var _document = commonAncestorContainer.ownerDocument;
            var dom = GlobalUtils.cloneElement(commonAncestorContainer);
            dom.innerHTML = '';
...
//            _document.body.appendChild(dom);
            var cloneDom = GlobalUtils.cloneElement(dom);
            cloneDom.removeAttribute('style');
            GlobalUtils.setAllStyleSrc2Target(dom, cloneDom);
            var result = cloneDom.outerHTML;
            $(dom).remove();
            dom = null;
            $(cloneDom).remove();
            cloneDom = null;
